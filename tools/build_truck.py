...
MATS = {
    ...
    "bt_seat":         ((0.20, 0.05, 0.05, 1), 0.0, 0.7, {}),
...
def build_front():
    o = obj("bt_body_front")
    fender = [(-0.74, 0.70), (-0.53, 0.70), (-0.53, 0.76), (-0.50, 0.86), (-0.43, 0.94), (-0.31, 0.99),
              (0.31, 0.99), (0.43, 0.94), (0.50, 0.86), (0.53, 0.76), (0.53, 0.66), (0.72, 0.66),
              (0.72, 1.362), (-0.74, 1.300)]
    ...
    for s in (1, -1):
        ...
        quad_x(o, "bt_under", s * 0.665, -0.53, 0.53, 0.66, 0.99, s)            # wheel-well liner
        # front corner of the fender, outboard of the headlights
        o.extrude_x("bt_paint", [(-0.80, 0.70), (-0.74, 0.70), (-0.74, 1.300), (-0.80, 1.297)],
        ...
        # fender crease: thin darker line under the top edge reads as the body line
...
def build_cab():
    o = obj("bt_body_cab")
    t = 0.05
    x1, y0, y1, z0, z1 = HALF_W, CAB_Y0, CAB_Y1, 0.64, BELT
    # --- lower tub (outer skin)
    ...
    for s in (1, -1):
        ...
        for y in (0.845, 1.835):  # door seams
            quad_x(o, "bt_black", xs, y - 0.003, y + 0.003, 0.66, BELT, s)
        quad_x(o, "bt_black", xs, 0.845, 1.835, 0.665, 0.671, s)
        # handle + lock
        box_s(o, "bt_chrome", s, HALF_W, HALF_W + 0.018, 1.60, 1.73, 1.285, 1.315)
        box_s(o, "bt_chrome", s, HALF_W, HALF_W + 0.006, 1.765, 1.785, 1.28, 1.30)
        # rocker shading
        quad_x(o, "bt_under", xs, 0.72, 1.98, 0.64, 0.665, s)
        # mirror: two arms + head
        for z in (1.33, 1.43):
            box_s(o, "bt_chrome", s, HALF_W, 1.14, 0.90, 0.915, z, z + 0.014)
...
def build_bed():
    o = obj("bt_bed")
    side = [(BED_Y0, 0.66), (2.80, 0.66), (2.80, 0.76), (2.83, 0.86), (2.90, 0.94), (3.02, 0.99),
            (3.66, 0.99), (3.78, 0.94), (3.85, 0.86), (3.88, 0.76), (3.88, 0.66), (BED_Y1, 0.66),
            (BED_Y1, BED_TOP), (BED_Y0, BED_TOP)]
    ...
    for s in (1, -1):
        ...
        quad_x(o, "bt_under", s * 0.905, 2.80, 3.88, 0.66, 0.99, s)             # wheel-well liner
        box_s(o, "bt_paint", s, 0.885, HALF_W + 0.004, BED_Y0, BED_Y1, BED_TOP, BED_TOP + 0.016)  # rail cap
        box_s(o, "bt_paint", s, 0.84, 0.91, 4.28, BED_Y1, 0.70, BED_TOP)                          # rear post
        # tail lamp: chrome bezel, red lens, clear backup lens
        box_s(o, "bt_chrome", s, 0.842, 0.988, BED_Y1, BED_Y1 + 0.012, 0.925, 1.325)
        box_s(o, "bt_taillight", s, 0.852, 0.978, BED_Y1 + 0.012, BED_Y1 + 0.018, 1.015, 1.315)
        box_s(o, "bt_lamp_clear", s, 0.852, 0.978, BED_Y1 + 0.012, BED_Y1 + 0.018, 0.935, 1.008)
        # rear side marker (red) and front side marker (amber) on the bed
        quad_x(o, "bt_taillight", s * (HALF_W + 0.002), 4.20, 4.30, 0.90, 0.94, s)
        # fuel door
        xs = s * (HALF_W + 0.002)
        quad_x(o, "bt_black", xs, 2.495, 2.655, 0.945, 1.085, s)
        quad_x(o, "bt_paint", s * (HALF_W + 0.004), 2.50, 2.65, 0.95, 1.08, s)
    ...
    # tailgate
    tg = obj("bt_tailgate")
    tg.box("bt_paint", (-0.84, 4.36, 0.72), (0.84, 4.44, 1.39))
    tg.box("bt_paint", (-0.82, 4.44, 0.78), (0.82, 4.447, 1.035))    # lower stamped panel
...
def build_bumpers():
    ...
    quad_y(r, "bt_plate", 4.6135, 0.058, 0.367, 0.525, 0.678, 1, uv=((0, 0), (1, 0), (1, 1), (0, 1)))
...
def make_wheel(rear):
    ...
    t.lathe("bt_tire", o, ax, [(p[0], -p[1]) for p in reversed(side)], segs=48)
    # rim
    ...
    if rear:
        ...
        w.lathe("bt_rust", o, ax, [(0.072, 0.035), (0.068, 0.10), (0.05, 0.105), (0.048, 0.118),
                                   (0.0, 0.118)], segs=24)
        w.box("bt_rust", (0.116, -0.008, -0.035), (0.121, 0.008, 0.035))  # hub lock dial knob
    # 8 lug nuts
    for k in range(8):
        a = 2 * math.pi * k / 8
...
def build_wheels():
    for name, (x, y), rear in (("FL", (TRACK_F, 0.0), False), ("FR", (-TRACK_F, 0.0), False),
        ...
        OBJS[f"bt_tire_{name}"] = t.transformed(mv, name=f"bt_tire_{name}")
...
def build_toolbox():
    ...
    y0, y1 = 2.09, 2.56
    zr = BED_TOP + 0.016
    o.box("bt_diamond", (-1.00, y0, zr), (1.00, y1, zr + 0.20))
    o.box("bt_diamond", (-0.875, y0, 1.12), (0.875, y1 - 0.08, zr))
    o.box("bt_diamond", (-0.99, y0 - 0.005, zr + 0.20), (0.99, y1 + 0.01, zr + 0.215))   # lid
    quad_y(o, "bt_black", y1 + 0.0105, -0.99, 0.99, zr + 0.196, zr + 0.2, 1)           # lid seam
    for s in (1, -1):
        box_s(o, "bt_black", s, 0.30, 0.40, y1, y1 + 0.02, zr + 0.13, zr + 0.18)       # latches
    o.box("bt_uws", (-0.045, y1, zr + 0.14), (0.045, y1 + 0.004, zr + 0.175))          # logo
...
def build_plowmount():
    ...
    o = obj("bt_plowmount")
    for s in (1, -1):
        box_s(o, "bt_frame", s, 0.38, 0.46, -1.12, -0.80, 0.40, 0.50)       # push beams to frame horns
        box_s(o, "bt_frame", s, 0.33, 0.39, -1.16, -1.10, 0.40, 1.34)       # light towers
        box_s(o, "bt_frame", s, 0.20, 0.40, -1.13, -1.00, 1.05, 1.08)       # tower braces
        # lamp stalk + housing (black back, chrome rim, clear lens, amber turn lens)
        box_s(o, "bt_rust", s, 0.52, 0.54, -1.15, -1.13, 1.37, 1.42)
        box_s(o, "bt_black", s, 0.38, 0.70, -1.17, -1.03, 1.42, 1.58)
        box_s(o, "bt_chrome", s, 0.385, 0.695, -1.175, -1.17, 1.425, 1.575)
        box_s(o, "bt_headlight", s, 0.40, 0.60, -1.18, -1.175, 1.44, 1.565)
        box_s(o, "bt_amber", s, 0.605, 0.685, -1.18, -1.175, 1.44, 1.565)
    o.box("bt_frame", (-0.72, -1.17, 1.34), (0.72, -1.10, 1.37))            # light bar
    o.box("bt_frame", (-0.52, -1.24, 0.34), (0.52, -1.10, 0.46))            # receiver cross beam
    ...
    o.cylinder("bt_black", (-0.02, -1.14, 0.50), (0.0, -1.14, 1.20), 0.035, segs=10)  # lift ram
    o.box("bt_frame", (-0.36, -1.16, 1.18), (0.36, -1.10, 1.22))
